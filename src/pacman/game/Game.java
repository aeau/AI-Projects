...
//		pacman=new PacMan(getActivePillsIndices()[rnd.nextInt(getActivePillsIndices().length)],MOVE.LEFT,NUM_LIVES,false);		
	...
	public int[] GetIntersections()
	{
		if(this.intersections == null || this.intersections.length == 0)
		{
			...
			int max_nodes = getNumberOfNodes();
			
			for(int i = 0; i < max_nodes; i++)
			{
				int neighbor_n = getNeighbour(i, MOVE.UP);
				int neighbor_e = getNeighbour(i, MOVE.RIGHT);
				int neighbor_s = getNeighbour(i, MOVE.DOWN);
				int neighbor_w = getNeighbour(i, MOVE.LEFT);
	...
	{
		double result = ((double)value - min)/(max - min);
		return result;
	}
	
	public String GetNeuralNetworkInfo(boolean null_movement)
	{
		if(pacman.lastMoveMade == MOVE.NEUTRAL && null_movement)
		{
			return "null";
		}
		
		StringBuilder sb=new StringBuilder();
		double normalize_value = 0.0;
		for(Ghost ghost : ghosts.values())
		{
			if(ghost.lairTime == 0)
			{
				normalize_value = NormalizeBetweenZeroAndOne(getShortestPathDistance(pacman.currentNodeIndex, ghost.currentNodeIndex), 0.0, 225.0);
				sb.append(normalize_value + ";");
			}
			else
			{
				sb.append("0.5;");
		...
		if(activePowerPills.length != 0)
		{
			...
			normalize_value = NormalizeBetweenZeroAndOne(getShortestPathDistance(pacman.currentNodeIndex, 
								getClosestNodeIndexFromNodeIndex (pacman.currentNodeIndex ,targetNodeIndices,DM.PATH)), 0.0, 225.0);
			sb.append(normalize_value + ";");
		}
		else
		{
		...
		normalize_value = NormalizeBetweenZeroAndOne(getShortestPathDistance(pacman.currentNodeIndex, 
													getClosestNodeIndexFromNodeIndex(pacman.currentNodeIndex,targetsArray,DM.PATH)),0.0 , 225.0);
		sb.append(normalize_value + ";");
		
		//Class of the tuple
		sb.append(pacman.lastMoveMade);
